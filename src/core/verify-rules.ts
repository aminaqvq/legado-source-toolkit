...
export async function verifyRuleBookInfo(
  ...
  if (!nameRule) {
    return makeResult('bookInfo', 'RULE_NOT_CHECKED', startTime, logs, 'No ruleBookInfo.name defined', undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }

  try {
    // Resolve URL
    const resolvedUrl = resolveCandidateUrl(bookUrl, {
      baseUrl: source.bookSourceUrl ?? '',
      sourceBaseUrl: source.bookSourceUrl ?? '',
    });
    if (!resolvedUrl) {
      errors.push('invalid_url');
      return makeResult('bookInfo', 'RULE_PARSE_ERROR', startTime, logs, `Cannot resolve book URL: ${bookUrl}`, undefined, undefined, undefined, undefined, rules, errors, suggestions);
    }
    ...
    const pageUrl = fetchResult.finalUrl || resolvedUrl;

    if (!fetchResult.ok || isErrorBody(fetchResult.body)) {
      mapFetchFailure(fetchResult, errors);
      return makeResult('bookInfo', 'RULE_EMPTY_RESULT', startTime, logs, 'Invalid response', resolvedUrl, fetchResult.body.length, undefined, undefined, rules, errors, suggestions, netTrace);
    }

    // Execute book info rules
    const bookInfo = extractBookInfo(source, fetchResult.body, pageUrl, logs, rules, errors, suggestions);

    if (!bookInfo.name) {
      errors.push('rule_empty_result');
      suggestions.push('检查 ruleBookInfo.name 是否匹配书籍详情页');
      return makeResult(
        'bookInfo', 'RULE_EMPTY_RESULT', startTime, logs,
        'name rule returned empty',
        resolvedUrl, fetchResult.body.length, 0, undefined, rules, errors, suggestions, netTrace,
      );
    }
    ...
    return {
      ...
      responseSize: fetchResult.body.length,
      resultCount: 1,
      resultSample: `name="${bookInfo.name}", author="${bookInfo.author ?? ''}"`,
      duration: Date.now() - startTime,
      logs,
      extracted: bookInfo,
      request: netTrace,
...
export async function verifyRuleToc(
  ...
  if (!chapterListRule) {
    return makeResult('toc', 'RULE_NOT_CHECKED', startTime, logs, 'No ruleToc.chapterList defined', undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }

  try {
    const resolvedUrl = resolveCandidateUrl(tocUrl, {
      baseUrl: source.bookSourceUrl ?? '',
      sourceBaseUrl: source.bookSourceUrl ?? '',
    });
    if (!resolvedUrl) {
      errors.push('invalid_url');
      return makeResult('toc', 'RULE_PARSE_ERROR', startTime, logs, `Cannot resolve TOC URL: ${tocUrl}`, undefined, undefined, undefined, undefined, rules, errors, suggestions);
    }
    ...
    if (!fetchResult.ok || isErrorBody(fetchResult.body)) {
      mapFetchFailure(fetchResult, errors);
      return makeResult('toc', 'RULE_EMPTY_RESULT', startTime, logs, 'Invalid response', resolvedUrl, fetchResult.body.length, undefined, undefined, rules, errors, suggestions, netTrace);
    }

    // Execute chapterList rule
    ...
    if (listResult.list.length === 0) {
      errors.push('rule_empty_result');
      suggestions.push('检查 ruleToc.chapterList 是否匹配目录列表');
      return makeResult('toc', 'RULE_EMPTY_RESULT', startTime, logs, 'chapterList returned 0 results', resolvedUrl, fetchResult.body.length, 0, undefined, rules, errors, suggestions, netTrace);
    }

    logs.push(`chapterList: ${listResult.list.length} chapters`);

    // ── Extract chapter items with item-scope rules ──
    const chapters = extractChapterItems(source, fetchResult.body, listResult, pageUrl, logs, rules, errors, suggestions);
    ...
    errors.push('network_error');
    return makeResult('toc', 'RULE_PARSE_ERROR', startTime, logs, `Error: ${err instanceof Error ? err.message : String(err)}`, undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }
}
...
export async function verifyRuleContent(
  ...
  if (!contentUrl) {
    return makeResult('content', 'RULE_NOT_CHECKED', startTime, logs, 'No contentUrl provided', undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }

  const contentRuleStr = source.ruleContent?.content;
  if (!contentRuleStr) {
    return makeResult('content', 'RULE_NOT_CHECKED', startTime, logs, 'No ruleContent.content defined', undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }
  ...
  try {
    ...
    if (!resolvedUrl) {
      errors.push('invalid_url');
      return makeResult('content', 'RULE_PARSE_ERROR', startTime, logs, `Cannot resolve content URL: ${contentUrl}`, undefined, undefined, undefined, undefined, rules, errors, suggestions);
    }
    ...
    if (!fetchResult.ok || isErrorBody(fetchResult.body)) {
      mapFetchFailure(fetchResult, errors);
      return makeResult('content', 'RULE_EMPTY_RESULT', startTime, logs, 'Invalid response', resolvedUrl, fetchResult.body.length, undefined, undefined, rules, errors, suggestions, netTrace);
    }
    ...
    // 20-100 chars: mark as LIKELY_PASS with warning (not full FAIL)
    // This is handled in stageResults, but for RuleVerifyDetail status we still use VERIFIED
    ...
    errors.push('network_error');
    return makeResult('content', 'RULE_PARSE_ERROR', startTime, logs, `Error: ${err instanceof Error ? err.message : String(err)}`, undefined, undefined, undefined, undefined, rules, errors, suggestions);
  }
}

// ══════════════════════════════════════════════════════
//  5. verifyAllRules — orchestrator (v1.5 real chain)
// ══════════════════════════════════════════════════════
...
export async function verifyAllRules(
  ...
  const searchItems = (searchStep.extracted as any)?.items as SearchItem[] | undefined;
  ...
  const bookInfo = infoStep.extracted as BookInfoResult | undefined;
  const tocUrl = bookInfo?.tocUrl || infoStep.url || '';
  const tocWarnings: string[] = [];
  if (!bookInfo?.tocUrl && infoStep.url) {
  ...
  const tocExtracted = tocStep.extracted as any;
  const chapters = tocExtracted?.chapters as ChapterItem[] | undefined;
