...
.card .num { font-size: 32px; font-weight: bold; color: #1a73e8; }
...
.tooltip:hover::after { content: attr(data-tip); position: absolute; bottom: 100%; left: 0; background: #333; color: #fff; padding: 4px 8px; border-radius: 4px; font-size: 11px; white-space: nowrap; z-index: 10; }
</style>
</head>
<body>
<div class="container">
...
<h2>📂 分类统计</h2>
<div class="cards">
  ${Object.entries(summary.input.categoryCounts).map(([cat, count]) =>
    `<div class="card"><div class="num">${count}</div><div class="label">${cat}</div></div>`
  ).join('')}
</div>
...
${duplicates.map((d) => {
  ...
  const removedNames = d.removedIndices.map((i) => sources[i]?.originalName || '?').join(', ');
  return `<tr>
    <td>${d.groupId}</td>
    <td>${escHtml(d.groupKey)}</td>
    <td>${escHtml(kept?.originalName || `#${d.keptIndex}`)} (score: ${kept?.score ?? '?'})</td>
    <td>${escHtml(removedNames)}</td>
...
function filterDupTable() {
  const q = document.getElementById('dupSearch').value.toLowerCase();
  document.querySelectorAll('#dupTable tbody tr').forEach(r => {
    r.style.display = r.textContent.toLowerCase().includes(q) ? '' : 'none';
  });
}
...
function filterAvailability(status, btn) {
  document.querySelectorAll('#availabilityFilter button').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');
  ['keptTable', 'removedTable'].forEach(tid => {
...
<thead><tr>
  ...
  <th>可用性</th><th>分数</th><th>保留</th><th>移除原因</th>
</tr></thead>
<tbody>
${sources.map((s) => {
